...
import { Building2Icon, CheckIcon, MinusIcon, SaveIcon, ServerCogIcon, ShieldCheckIcon, XIcon } from "lucide-react";
...
export function Parametres() {
  ...
  return (
    <div className="space-y-4">
      ...
      <Tabs value={onglet} onValueChange={(v) => setParams(v === "entreprise" ? {} : { onglet: v }, { replace: true })}>
          ...
          <>
            ...
            <TabsContent value="entreprise" className="grid gap-4 lg:grid-cols-[1fr_22rem]">
              ...
              <Bloc titre="Aperçu de l'en-tête" description="Tel qu'il apparaît sur un document imprimé." className="lg:row-span-2">
                <div className="rounded border border-filet bg-white p-4 text-encre shadow-xs">
            ...
            <TabsContent value="fonctionnement" className="grid gap-4 lg:grid-cols-2">
              ...
              <Bloc titre="Valeurs par défaut" description="Ce que vaut chaque réglage si vous n'y touchez pas.">
                ...
                <Button type="button" variant="outline" size="sm" className="mt-3" onClick={() => setR({ ...REGLAGES_DEFAUT })}>Remettre les valeurs par défaut</Button>
              </Bloc>
            </TabsContent>
          </>
        )}
...
function Bloc({ titre, description, className, children }: { titre: string; description?: string; className?: string; children: React.ReactNode }) {
  return (
    <section className={cn("rounded-xl border border-filet bg-surface p-4", className)}>
      <h2 className="text-sm font-semibold">{titre}</h2>
      {description && <p className="mb-3 text-2xs text-encre-pale">{description}</p>}
      {children}
    </section>
  );
}
...
function Interrupteur({ libelle, aide, valeur, onChange, modifie }: { libelle: string; aide?: string; valeur: boolean; onChange: (v: boolean) => void; modifie?: boolean }) {
  return (
    <label className={cn("mt-3 flex items-center justify-between gap-4 rounded-lg border border-filet bg-fond px-3 py-2", modifie && "border-ocre bg-ocre/5")}>
      <span>
  ...
  return (
    <>
      <p className="text-xs text-encre-douce">
        Les niveaux sont cumulatifs : un rôle a tout ce qu'ont les rôles en dessous. L'auditeur externe est hors hiérarchie (audit et tableau de bord seulement).
        Cette matrice est dérivée du code (<code className="font-mono">convex/rbac.ts</code>) : ce que l'écran affiche est exactement ce que le serveur applique.
      </p>
        ...
        <TableBody>
          {categories.map((cat) => (
            <React.Fragment key={cat}>
              <TableRow className="bg-fond hover:bg-fond">
                <TableCell colSpan={roles.length + 1} className="py-1.5 text-2xs font-semibold uppercase tracking-wide text-encre-douce">{cat}</TableCell>
              </TableRow>
      ...
      <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
        {roles.map((r) => (
          <div key={r} className="rounded-lg border border-filet bg-surface p-3">
            <div className="text-xs font-semibold">{LIBELLE[r]} <span className="font-mono text-2xs font-normal text-encre-pale">{NIVEAU[r] ? `niv. ${NIVEAU[r]}` : "hors hiérarchie"}</span></div>
            <div className="text-2xs text-encre-douce">{DESCRIPTION[r]}</div>
          </div>
        ))}
      </div>
...
function EtatDeploiement() {
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-2xs text-encre-pale">Les valeurs ne sont jamais affichées ni transmises au navigateur. Pour définir une variable : <code className="font-mono">npx convex env set NOM valeur</code> (depuis <code className="font-mono">app/</code>), puis redéployer si elle est lue par <code className="font-mono">auth.config.ts</code>.</p>
    </div>
  );
}
